	...
	private void btnCalcLoan(ActionEvent event) {
		for(int i=0;i<PaymentScheduleList.getColumns().size();++i){
			...
			if(number1.getText().equals("Payment #")){
				number1.setCellValueFactory(new PropertyValueFactory<LoanResolver.PaymentItem, String>("paymentNumber"));
			}else if(number1.getText().equals("Due Date")){
				number1.setCellValueFactory(new PropertyValueFactory<LoanResolver.PaymentItem, String>("date"));
			}else if(number1.getText().equals("Payment")){
				number1.setCellValueFactory(new PropertyValueFactory<LoanResolver.PaymentItem, String>("payment"));
			}else if(number1.getText().equals("Additonal Payment")){
				number1.setCellValueFactory(new PropertyValueFactory<LoanResolver.PaymentItem, String>("additionalPayment"));
			}else if(number1.getText().equals("Interest")){
				number1.setCellValueFactory(new PropertyValueFactory<LoanResolver.PaymentItem, String>("interest"));
			}else if(number1.getText().equals("Principle")){
				number1.setCellValueFactory(new PropertyValueFactory<LoanResolver.PaymentItem, String>("principle"));
			}else if(number1.getText().equals("Balance")){
				number1.setCellValueFactory(new PropertyValueFactory<LoanResolver.PaymentItem, String>("balance"));
			}
		}
